...
const fetchNextMe = async () => {
  debugLog("settings", "나의 목표를 조회합니다.");
  try {
    const result = await getNextMe();
    debugLog("settings", "나의 목표를 조회했습니다.", result);
    return result;
  } catch (error) {
    debugError("settings", "나의 목표 조회에 실패했습니다.", error);
    throw error;
  }
};
...
const GoalPage = () => {
  ...
  const handleRetry = async () => {
    if (updateError) {
      debugLog("settings", "나의 목표 수정을 다시 시도합니다.");
      const result = await retryUpdate();
      if (!result) {
        debugError("settings", "나의 목표 수정에 실패했습니다.");
        return;
      }
      debugLog("settings", "나의 목표를 수정했습니다.", result);
      applyUpdatedGoal(result);
      return;
    }

    debugLog("settings", "나의 목표 조회를 다시 시도합니다.");
    refetch();
  };
  ...
  const handleSubmit = (key) => async (newValue) => {
    ...
    if (nextValue === getSheetValue(key, data)) {
      debugLog("settings", "수정할 목표 정보가 없어 요청을 건너뜁니다.", {
        key,
        nextValue,
      });
      return;
    ...
    debugLog("settings", "나의 목표를 수정합니다.", body);
